...
class playGame extends Phaser.Scene {
  ...
  create() {
    const createNPC = (x, y, spriteName, spriteFrame, text, reference, battleScene) => {
      const npc = this.NPCs.create(x, y, spriteName, spriteFrame)
        .setSize(0, 38)
        .setOffset(0, 23);
      npc.text = text || '';
    ...
    this.physics.add.collider(player, this.NPCs, (player, spriteNPC) => {
      const _spriteNPC = spriteNPC;
      const directionObj = spriteNPC.body.touching;
      let direction = null;
      for (const key in directionObj) {
        if (directionObj[key]) {
          if (key === 'down') {
            direction = 'front';
          } else if (key === 'up') {
  ...
  update(time, delta) {
    ...
    if (!this.physics.paused) {
      ...
      if (cursors.left.isDown) {
        ...
        if (prevVelocity.x < 0) { player.setTexture('atlas', 'student-left'); } else if (prevVelocity.x > 0) { player.setTexture('atlas', 'student-right'); } else if (prevVelocity.y < 0) { player.setTexture('atlas', 'student-back'); } else if (prevVelocity.y > 0) { player.setTexture('atlas', 'student-front'); }
      }
    }
  }
}
